...
class Rexbuilder_Block {
	...
	public function render_block( $atts, $content = null ) {
		...
		if('true' == $builder_active) {
			...
			echo ( ( ('full' == $type_bg_block && "" != $id_image_bg_block && "" == $content  && $section_layout == 'fixed') || (!empty($video_bg_id) && "" == $content) || (!empty($video_bg_url) && "" == $content) ) ? ' only-background' : '' );
			...
			switch( $type ) :
				case 'image':
					...
					echo '<div class="grid-item-content image-content' . ( ('' != $video_bg_url && 'undefined' != $video_bg_url) ? ' youtube-player' : '' ) . ( ( '' != $video_bg_id && 'undefined' != $video_bg_id ) ? ' mp4-player' : '' ) . ( ($flex_positioned) ? ' rex-flexbox' : '' ) . '" ' . $section_style;
				...
				case 'text':
					echo $block_link_before;
					echo '<div class="grid-item-content text-content' . ( ('' != $video_bg_url && 'undefined' != $video_bg_url) ? ' youtube-player' : '' ) . ( ( '' != $video_bg_id && 'undefined' != $video_bg_id ) ? ' mp4-player' : '' ) . ( ($flex_positioned) ? ' rex-flexbox' : '' ) . '" ' . $section_style;
					echo $bg_youtube_video_markup;
					echo '>';
